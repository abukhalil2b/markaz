...
<head>
    ...
    <title>{{ $title ?? 'مؤسسة دار الإتقان العالي' }}</title>
...
<body x-data="main" class="antialiased relative font-nunito text-sm font-normal overflow-x-hidden" :class="[$store.app.sidebar ? 'toggle-sidebar' : '', $store.app.theme === 'dark' || $store.app.isDarkMode ?  'dark' : '', $store.app.menu, $store.app.layout, $store.app
        .rtlClass
    ]">
    ...
    <div class="screen_loader fixed inset-0 bg-[#fafafa] dark:bg-[#060818] z-[60] grid place-content-center animate__animated">
        <svg width="64" height="64" viewBox="0 0 135 135" xmlns="http://www.w3.org/2000/svg" fill="#dfbd8e">
    ...
    <div class="main-container text-black dark:text-white-dark min-h-screen" :class="[$store.app.navbar]">
        ...
        <div class="main-content flex flex-col min-h-screen">
            ...
            <x-common.impersonate />

            @if(App::environment(['local', 'testing', 'staging']))
            <div class="flex items-center justify-center">
                <div class="w-full p-1 font-bold text-orange bg-gray-400 text-sm text-center">
                    {{ App::environment() }}
                </div>
            </div>
            @endif

            <div class="p-3 animate__animated" :class="[$store.app.animation]">
                {{ $slot }}
            </div>
