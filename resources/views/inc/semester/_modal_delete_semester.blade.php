<div x-data="modal_delete_semester">
        ...
        <div class="flex min-h-screen items-center justify-center px-4" @click.self="open = false">
            <div x-show="open" x-transition x-transition.duration.300 class="panel my-8 w-full max-w-lg overflow-hidden rounded-lg border-0 p-0">
                <div class="flex items-center justify-between bg-[#fbfbfb] px-5 py-3 dark:bg-[#121c2c]">
                    <h5 class="text-lg font-bold"> {{ $semester->title }} </h5>
                ...
                <form method="post" action="{{ route('admin.semester.delete',['semester'=>$semester->id]) }}" class="p-4">
                    @csrf
                    <div class="p-5">

                        <div class="text-red-400">
                            كذلك سيتم حذف كافة المستحقات المرتبطة بهذا الفصل
                        </div>
...
<script>
    document.addEventListener("alpine:init", () => {
        ...
        Alpine.data("modal_delete_semester", () => ({
